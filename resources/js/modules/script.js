export function delegationClick() {
    ...
    //отслеживание скролла для всплывашки
    const currentScrollY = window.scrollY;
    let timerPopupMain;
    window.addEventListener("scroll", eventHandlerScroll);
    function eventHandlerScroll() {
        if (currentScrollY + 100 <= window.scrollY) {
            const popupPreview = document.querySelector("#window-contact-preview");
            popupPreview.classList.add("active");
            timerPopupMain = setTimeout(() => {
                popupPreview.classList.remove("active");
                const popupMain = document.querySelector("#window-contact-main");
                popupMain.classList.add("active");
            }, 7000);
            window.removeEventListener("scroll", eventHandlerScroll);
        }
    }
    ...
    function documentActions(e) {
        ...
        //для контактного окошка
        if (targetElement.closest("#window-contact-close")) {
            const popupPreview = document.querySelector("#window-contact-preview");
            popupPreview.classList.add("active");
            const popupMain = document.querySelector("#window-contact-main");
            popupMain.classList.remove("active");
            if (timerPopupMain) {
                clearTimeout(timerPopupMain);
            }
        }
        if (targetElement.closest("#window-contact-preview")) {
            const popupPreview = document.querySelector("#window-contact-preview");
            popupPreview.classList.remove("active");
            const popupMain = document.querySelector("#window-contact-main");
            popupMain.classList.add("active");
